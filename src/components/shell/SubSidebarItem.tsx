...
export const SubSidebarItem: React.FC<SubSidebarItemProps> = ({
    ...
    return (
      <Tooltip content={title} placement="right" delay={200}>
        ...
        >
          ...
          {badge && (
            <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-primary ring-2 ring-[#0F0F0F]" />
          )}
        </Link>
      </Tooltip>
    );
  ...
  return (
    <Link
      to={href}
      onClick={onClick}
      className={`flex items-center justify-between min-h-[34px] h-[34px] px-2.5 rounded-xl text-[13px] transition-all duration-150 cursor-pointer box-border leading-tight shrink-0 select-none ${
        isActive
          ? 'bg-primary text-white font-bold shadow-xs border border-primary/40'
          : 'text-slate-400 font-medium hover:text-slate-100 hover:bg-white/[0.06] border border-transparent'
      }`}
    >
      <div className="flex items-center gap-2 min-w-0">
        {Icon && (
          <Icon className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-white' : 'text-slate-500 group-hover:text-slate-300'}`} />
        )}
